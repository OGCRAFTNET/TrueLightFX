...
float CloudSampleBaseModern(vec2 coord) {
    // Maximale Performance durch minimierte Berechnungen
    vec2 coord1 = coord * 0.25;
    float mainNoise = texture2D(noisetex, coord1).r;
    
    // Schnelle Kontrast- und Helligkeitsanpassung
    float noise = mainNoise * 0.7 + 0.3;
    noise = noise * noise * (3.0 - 2.0 * noise); // Optimierte Smooth-Funktion
    
    // Schnellere Alternative zu pow() für bessere Performance
    noise = noise * noise * 1.8;
...
float CloudSampleDetail(vec2 coord, float cloudGradient) {
	// Optimierte Detail-Berechnung mit weniger Texture-Lookups
	float thickness = float(CLOUD_THICKNESS);
	float detailZ = floor(cloudGradient * thickness) * 0.04;
	
	// Schnellere Frac-Berechnung
	float detailFrac = cloudGradient * thickness - floor(cloudGradient * thickness);
	
	// Optimierte Texturkoordinaten
	vec2 baseCoord = coord.xy + detailZ;
	
	// Single texture fetch optimization wenn möglich
	float noiseDetail;
	if (detailFrac < 0.1) {
		noiseDetail = texture2D(noisetex, baseCoord).b;
	} else {
		float noiseDetailLow = texture2D(noisetex, baseCoord).b;
		float noiseDetailHigh = texture2D(noisetex, baseCoord + 0.04).b;
		noiseDetail = mix(noiseDetailLow, noiseDetailHigh, detailFrac);
	}
...
float CloudSample(vec2 coord, vec2 wind, float cloudGradient, float dither) {
	// Optimierte Koordinatenberechnung
	coord *= 0.004 * CLOUD_STRETCH;
	vec2 windOffset = wind * vec2(1.0, 0.8); // Reduzierte Wind-Berechnungen

	#if CLOUD_BASE == 0
	// Vorberechnete Koordinaten für bessere Performance
	vec2 baseCoord = coord * 0.25 + windOffset;
	vec2 detailCoord = coord * 0.5 - windOffset;

	// Cache Gradienten-basierte Werte
	float gradientFactor = max(0.0, min(1.0, cloudGradient));
	float noiseBase = CloudSampleBasePerlin(baseCoord);
	float noiseDetail = CloudSampleDetail(detailCoord, gradientFactor);
	float noiseCoverage = CloudCoverageDefault(gradientFactor);
	...
	float noiseBase = CloudSampleBaseModern(baseCoord);
	float noiseDetail = CloudSampleDetail(detailCoord, cloudGradient);
	float noiseCoverage = CloudCoverageDefault(cloudGradient);

	float noise = CloudCombineDefault(noiseBase, noiseDetail, noiseCoverage);
	
	// Verbesserte Farbdynamik und Kontraste
	noise = pow(noise, 1.15) * 1.35; // Hellere Highlights
	noise = mix(noise, noise * noise, 0.3); // Verstärkter Kontrast
	noise *= 1.1; // Leicht erhöhte Gesamthelligkeit
...
void DrawStars(inout vec3 color, vec3 viewPos) {
	...
	float multiplier = sqrt(sqrt(VoU)) * 5.0 * (1.0 - rainStrength) * (1.0 - sunVisibility);
	
	float star = 1.0;
	if (VoU > 0.0 && sunVisibility < 0.1) {
