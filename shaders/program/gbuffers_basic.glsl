...
void main() {
	...
	#if BLOCK_OUTLINE_MODE > 0
	if (color.a < 0.9) { // Block selection outline has low alpha
		#if BLOCK_OUTLINE_MODE == 1
		// Custom Color
		albedo.rgb = vec3(BLOCK_OUTLINE_R, BLOCK_OUTLINE_G, BLOCK_OUTLINE_B);
		albedo.a = BLOCK_OUTLINE_OPACITY;
		#elif BLOCK_OUTLINE_MODE == 2
		// Rainbow
		float hue = fract(frameTimeCounter * BLOCK_OUTLINE_ANIM_SPEED * 0.1);
		float s = 1.0, v = 1.0;
		float c = v * s;
		...
		albedo.rgb = rgb + m;
		albedo.a = BLOCK_OUTLINE_OPACITY;
		#elif BLOCK_OUTLINE_MODE == 3
		// Gradient
		float t = fract(frameTimeCounter * BLOCK_OUTLINE_ANIM_SPEED * 0.05);
		vec3 baseCol = vec3(BLOCK_OUTLINE_R, BLOCK_OUTLINE_G, BLOCK_OUTLINE_B);
		float shift = sin(t * 6.28318) * 0.5 + 0.5;
		vec3 gradCol = mix(baseCol, vec3(1.0) - baseCol, shift);
		albedo.rgb = gradCol;
		albedo.a = BLOCK_OUTLINE_OPACITY;
		#endif
	}
